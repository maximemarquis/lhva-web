...
export default async function AdminDashboard() {
  ...
  const [gamesRes, articlesRes, playersRes] = await Promise.all([
    supabase.from('games').select('id, is_published, home_score').eq('season_id', 1),
  ...
  const gamesPlayed  = games.filter(g => g.home_score !== null && g.is_published).length
  const gamesPending = games.filter(g => g.home_score === null).length
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-rink-800 border border-white/[0.07] rounded-lg overflow-hidden">
          ...
          <div className="p-4 grid grid-cols-2 gap-3">
            {[
              { label: 'Enter game score', href: '/admin/scores',   desc: 'Goals, scorers, time' },
